...
def delete_redundant_video_keys(data):
    del data['abr']
    del data['acodec']
    ...
    del data['webpage_url']
    del data['was_live']
    del data['_has_drm']
    del data['_format_sort_fields']


def yt_dlp_extract_video_information(video_url, print_info=False):
    with yt_dlp.YoutubeDL({}) as ydl:
        video_info = ydl.extract_info(video_url, download=False)
        delete_redundant_video_keys(video_info)
        if print_info:
            pprint.pprint(video_info, width=124)
    return video_info
