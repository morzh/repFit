...
def hook_skeleton_h36m_curves(skeleton_curves: dict[str, any], 
                              joints_spheres: list) -> None:
    for curve_name in skeleton_curves.keys():
        match curve_name:
            case 'LeftLegCurve':
                left_leg_hooks = {0: joints_spheres[0], 
                                  ...
                                  2: joints_spheres[2], 
                                  3: joints_spheres[3]}
                hook_skeleton_curve(skeleton_curves[curve_name], left_leg_hooks)
            case 'RightLegCurve':
                right_leg_hooks = {0: joints_spheres[0], 
                                   1: joints_spheres[4], 
                                   2: joints_spheres[5], 
                                   3: joints_spheres[6]}
                hook_skeleton_curve(skeleton_curves[curve_name], right_leg_hooks)
            case 'TorsoHeadCurve':
                torso_head_hooks = {0: joints_spheres[0], 
                                    ...
                                    3: joints_spheres[9], 
                                    4: joints_spheres[10]}
                hook_skeleton_curve(skeleton_curves[curve_name], torso_head_hooks)
            case 'LeftArmCurve':
                left_arm_hooks = {0: joints_spheres[8], 
                                  1: joints_spheres[14], 
                                  2: joints_spheres[15], 
                                  3: joints_spheres[16]}
                hook_skeleton_curve(skeleton_curves[curve_name], left_arm_hooks)
            case 'RightArmCurve':
                right_arm_hooks = {0: joints_spheres[8], 
                                   1: joints_spheres[11], 
                                   2: joints_spheres[12], 
                                   3: joints_spheres[13]}
                hook_skeleton_curve(skeleton_curves[curve_name], right_arm_hooks)
...
def hook_skeleton_curve(curve_object: any, hooks: dict[int, str]) -> None:
    ...
    for curve_point_index, hook_object in hooks.items():
        ...
        current_point = curve_object.data.splines.active.points[curve_point_index]
...
def create_constant_video_material(video_file_pathname: str) -> any:
    ...
    texture.image_user.current_video_frame = bpy.data.images.load(video_file_pathname)
...
def create_skeleton_video_sample(root_folder, video_filename) -> Type[bpy.context.active_object]:
    root_skeleton_data_folder = os.path.join(root_folder, 'MotionBERT') 
    root_videos_folder = os.path.join(root_folder, 'AlphaPose', 'video')
    
    joints_animation_filename = video_filename + '.json.npy'
    joints_animation_file_pathname = os.path.join(root_skeleton_data_folder, joints_animation_filename)
    video_file_pathname = os.path.join(root_videos_folder, 'AlphaPose_' + video_filename)

    if not os.path.exists(joints_animation_file_pathname):
        exit()

    skeleton_animation_curves = np.load(joints_animation_file_pathname)
    number_joints_frames = skeleton_animation_curves.shape[0]
    number_skeleton_joints = skeleton_animation_curves.shape[1] # 17
    initial_joints_location = skeleton_animation_curves[0]

    print(BColors.OKBLUE + BColors.BOLD, 'filename:', BColors.ENDC, 
          BColors.OKGREEN, joints_animation_filename, BColors.ENDC, 
          BColors.OKBLUE + BColors.BOLD, '; number skeleton frames:', BColors.ENDC, 
          BColors.WARNING, skeleton_animation_curves.shape[0], BColors.ENDC)
    ...
    bpy.context.scene.frame_end = number_joints_frames
    ...
    video_plane = get_video_plane(video_file_pathname)
    joints_skeleton_material = create_joints_skeleton_material()
    video_plane_material = create_constant_video_material(video_file_pathname)
    ...
    for joint_index in range(number_skeleton_joints):
        ...
        for frame_index in range(number_joints_frames):
            joints_spheres[joint_index].location = skeleton_animation_curves[frame_index, joint_index, :]
...
def main():
    root_folder = \
        '/media/anton/4c95a564-35ea-40b5-b747-58d854a622d0/home/anton/work/fitMate/dataset_aggregation/squats_2022_skeletons/data'

    # video_files = [x for x in glob.glob(root_folder)]
    video_files = [f for f in os.listdir(os.path.join(root_folder, 'AlphaPose', 'video'))]
    
    video_files = [
        'Squat Bodyweight-zawXRtyOTuA.mp4',
        'stephanie doing kettlebell clean and squat to press-YduwRC64IQo.mp4',
        'zercher squats-Nr3FcuYJJBI.mp4',
        'Приседания ноги узко 200_15-GyPdjhQF3lk.mp4',
        'Squat Training, 195x8-RxWaDZd8bp8.mp4',
        'Split Squat-aH7CsxXDj5w.mp4',
        'R Sindelar ohead squat-PrX15kDXhQ8.mp4',
    ]
    

    for video_index, video_file in enumerate(video_files):
        # video_filename = video_file[10:]
        video_filename = video_file
        sample_frame = create_skeleton_video_sample(root_folder, video_filename)
        sample_frame.location[1] = 5.0 * video_index
