import sys
# sys.path.append('/home/anton/work/fitMate/repFitDraft/.venv/lib/python3.11/site-packages')
# import ffmpeg
import cv2
import json
import bpy
import bmesh
import glob
import os.path
from typing import Type, List, Optional
from dataclasses import dataclass
import numpy as np
# from blender3d.utils import *


@dataclass(frozen=True, slots=True)
class H36mVideoSample:
    video_filepath: Optional[str] = None


@dataclass(frozen=True, slots=True)
class H36mSkeletonAnimationSample:
    H36M_NUMBER_JOINTS = 17

    video_filepath: Optional[str] = None
    skeleton_animation_filepath: Optional[str] = None
    skeleton_animation_stacked_filepath: Optional[str] = None
    skeleton_animation_pca_filepath: Optional[str] = None

    def get(self):
        coordinate_frame = self.sample_coordinate_frame()
        return coordinate_frame

    def sample_coordinate_frame(self):
        """

        @param joints_animation_pca:
        @param joints_filename:
        @return:
        """
        if self.video_filepath is not None:
            coordinate_frame_name = os.path.basename(self.video_filepath)
        elif self.skeleton_animation_filepath is not None:
            coordinate_frame_name = os.path.basename(self.skeleton_animation_filepath)
        elif self.skeleton_animation_stacked_filepath is not None:
            coordinate_frame_name = os.path.basename(self.skeleton_animation_stacked_filepath)
        else:
            coordinate_frame_name = os.path.basename(self.skeleton_animation_pca_filepath)

        bpy.ops.object.empty_add(type='ARROWS', location=(0, 0, 0))
        bpy.context.active_object.name = coordinate_frame_name
        coordinate_frame = bpy.context.active_object
        if self.skeleton_animation_pca_filepath is not None:
            coordinate_frame.data['pca'] = 0

        return coordinate_frame


H36M_NUMBER_JOINTS = 17


class BColors:
    HEADER = '\033[95m'
    OKBLUE = '\033[94m'
    OKCYAN = '\033[96m'
    OKGREEN = '\033[92m'
    WARNING = '\033[93m'
    FAIL = '\033[91m'
    ENDC = '\033[0m'
    BOLD = '\033[1m'
    UNDERLINE = '\033[4m'


# def defaults_modifier(mod):
#     mod.show_in_editmode = True
#     mod.show_on_cage = True
#
def get_h36m_joints_colors() -> list[Type[tuple]]:
    number_joints = 17
    joints_colors = [tuple[float, float, float, float]] * number_joints
    light_blue = (0.25, 0.25, 1.0, 1.0)
    light_green = (0.25, 1.0, 0.25, 1.0)
    orange = (0.7, 0.5, 0.1, 1.0)
    red = (1.0, 0.25, 0.25, 1.0)

    joints_colors[0] = orange  # root

    joints_colors[1] = light_blue  # left leg
    joints_colors[2] = light_blue  # left leg
    joints_colors[3] = light_blue  # left leg

    joints_colors[4] = light_green  # right leg
    joints_colors[5] = light_green  # right leg
    joints_colors[6] = light_green  # right leg

    joints_colors[7] = orange  # spine
    joints_colors[8] = orange  # spine

    joints_colors[9] = red  # head
    joints_colors[10] = red  # head

    joints_colors[11] = light_green  # right arm
    joints_colors[12] = light_green  # right arm
    joints_colors[13] = light_green  # right arm

    joints_colors[14] = light_blue  # left arm
    joints_colors[15] = light_blue  # left arm
    joints_colors[16] = light_blue  # left arm

    return joints_colors


def get_h36m_joints_names():
    h36m_number_joints = 17
    joints_names = [str] * h36m_number_joints

    joints_names[0] = '00 Bottom Torso'

    joints_names[1] = '01 Left Hip'
    joints_names[2] = '02 Left Knee'
    joints_names[3] = '03 Left Foot'

    joints_names[4] = '04 Right Hip'
    joints_names[5] = '05 Right Knee'
    joints_names[6] = '06 Right Foot'

    joints_names[7] = '07 Center Torso'
    joints_names[8] = '08 Upper Torso'

    joints_names[9] = '09 Neck Base'
    joints_names[10] = '10 Center Head'

    joints_names[11] = '11 Right Shoulder'
    joints_names[12] = '12 Right Elbow'
    joints_names[13] = '13 Right Hand'

    joints_names[14] = '14 Left Shoulder'
    joints_names[15] = '15 Left Elbow'
    joints_names[16] = '16 Left Hand'

    return joints_names


def get_h36m_skeleton_curves(joints_locations, joints_colors) -> dict[str, any]:
    assert joints_locations.shape == (17, 3)

    left_leg_initial_locations = joints_locations[:4]
    right_leg_initial_locations = np.vstack((joints_locations[0], joints_locations[4:7]))
    torso_head_locations = np.vstack((joints_locations[0], joints_locations[7:11]))
    left_arm_locations = np.vstack((joints_locations[8], joints_locations[14:17]))
    right_arm_locations = np.vstack((joints_locations[8], joints_locations[11:14]))

    left_leg_color = joints_colors[1]
    right_leg_color = joints_colors[4]
    torso_head_color = joints_colors[7]
    left_arm_color = joints_colors[14]
    right_arm_color = joints_colors[12]

    left_leg_curve = create_curve_3d('LeftLegCurve', left_leg_initial_locations, left_leg_color)
    right_leg_curve = create_curve_3d('RightLegCurve', right_leg_initial_locations, right_leg_color)
    torso_head_curve = create_curve_3d('TorsoHeadCurve', torso_head_locations, torso_head_color)
    left_arm_curve = create_curve_3d('LeftArmCurve', left_arm_locations, left_arm_color)
    right_arm_curve = create_curve_3d('RightArmCurve', right_arm_locations, right_arm_color)

    return {'LeftLegCurve': left_leg_curve, 'RightLegCurve': right_leg_curve,
            'TorsoHeadCurve': torso_head_curve,
            'LeftArmCurve': left_arm_curve, 'RightArmCurve': right_arm_curve}


def parent_skeleton_curves(skeleton_curves, parent) -> None:
    for curve_object in skeleton_curves.values():
        curve_object.parent = parent


def hook_skeleton_h36m_curves(skeleton_curves: dict[str, any], joints_spheres: list) -> None:
    for curve_name, data_object in skeleton_curves.items():
        match curve_name:
            case 'LeftLegCurve':
                left_leg_hooks = {0: joints_spheres[0],
                                  1: joints_spheres[1],
                                  2: joints_spheres[2],
                                  3: joints_spheres[3]}
                hook_skeleton_h36m_single_curve(data_object, left_leg_hooks)
            case 'RightLegCurve':
                right_leg_hooks = {0: joints_spheres[0],
                                   1: joints_spheres[4],
                                   2: joints_spheres[5],
                                   3: joints_spheres[6]}
                hook_skeleton_h36m_single_curve(data_object, right_leg_hooks)
            case 'TorsoHeadCurve':
                torso_head_hooks = {0: joints_spheres[0],
                                    1: joints_spheres[7],
                                    2: joints_spheres[8],
                                    3: joints_spheres[9],
                                    4: joints_spheres[10]}
                hook_skeleton_h36m_single_curve(data_object, torso_head_hooks)
            case 'LeftArmCurve':
                left_arm_hooks = {0: joints_spheres[8],
                                  1: joints_spheres[14],
                                  2: joints_spheres[15],
                                  3: joints_spheres[16]}
                hook_skeleton_h36m_single_curve(data_object, left_arm_hooks)
            case 'RightArmCurve':
                right_arm_hooks = {0: joints_spheres[8],
                                   1: joints_spheres[11],
                                   2: joints_spheres[12],
                                   3: joints_spheres[13]}
                hook_skeleton_h36m_single_curve(data_object, right_arm_hooks)


def create_curve_3d(curve_name: str, initial_locations: np.ndarray, color=(1., 1., 1., 1.), depth=0.01) -> any:
    curve_data = bpy.data.curves.new(curve_name, 'CURVE')
    curve_data.dimensions = '3D'

    spline = curve_data.splines.new('POLY')
    spline.points.add(len(initial_locations) - 1)

    for point_index, point in enumerate(spline.points):
        current_coordinates = (initial_locations[point_index, 0],
                               initial_locations[point_index, 1],
                               initial_locations[point_index, 2],
                               1)
        point.co = current_coordinates

    curve_object = bpy.data.objects.new(curve_name + 'Object', curve_data)
    curve_object.color = color
    curve_object.data.bevel_depth = 0.01
    curve_object.data.bevel_resolution = 10
    curve_object.data.splines[0].use_smooth = False

    view_layer = bpy.context.view_layer
    view_layer.active_layer_collection.collection.objects.link(curve_object)

    return curve_object


def clear_material(material):
    if material.node_tree:
        material.node_tree.links.clear()
        material.node_tree.nodes.clear()


def assign_material_skeleton_curves(skeleton_curves: dict, joints_skeleton_material):
    for curve in skeleton_curves.values():
        curve.data.materials.append(joints_skeleton_material)


def hook_skeleton_h36m_single_curve(curve_object: any, hooks: dict[int, str]) -> None:
    # assert len(curve_object.points) == len(hooks)
    bpy.ops.object.select_all(action='DESELECT')
    # bpy.context.space_data.context = 'MODIFIER'

    bpy.context.view_layer.objects.active = curve_object
    bpy.ops.object.mode_set(mode='EDIT')

    for curve_point_index, hook_object in hooks.items():
        bpy.ops.object.modifier_add(type='HOOK')
        current_hook_name = 'Hook' if curve_point_index == 0 else 'Hook.{:03d}'.format(curve_point_index)
        bpy.context.object.modifiers[current_hook_name].object = hook_object

        current_point = curve_object.data.splines[0].points[curve_point_index]
        current_point.select = True
        bpy.ops.object.hook_assign(modifier=current_hook_name)
        current_point.select = False

    bpy.ops.object.mode_set(mode='OBJECT')


def create_joints_skeleton_material() -> any:
    materials = bpy.data.materials
    material = materials.new('joints_skeleton_matrial')
    material.use_nodes = True
    clear_material(material)
    nodes = material.node_tree.nodes
    links = material.node_tree.links

    object_info = nodes.new(type='ShaderNodeObjectInfo')
    diffuse = nodes.new(type='ShaderNodeBsdfDiffuse')
    output = nodes.new(type='ShaderNodeOutputMaterial')

    links.new(diffuse.outputs['BSDF'], output.inputs['Surface'])
    links.new(object_info.outputs['Color'], diffuse.inputs['Color'])

    return material


def get_video_plane(video_file_pathname: str) -> any:
    video_capture = cv2.VideoCapture(video_file_pathname)
    video_height = video_capture.get(cv2.CAP_PROP_FRAME_HEIGHT)
    video_width = video_capture.get(cv2.CAP_PROP_FRAME_WIDTH)
    # print(video_height, video_width)

    video_plane_scale = 2 * np.array([video_width, video_height]) / video_height
    # video_plane_scale = 2 * np.array([video_width, video_height]) / max(video_height, video_width)
    bpy.ops.mesh.primitive_plane_add(size=1,
                                     location=(1. + 0.5 * video_plane_scale[0], 0., 0.),
                                     rotation=(0., 0., 0.))
    video_plane_object = bpy.context.active_object
    video_plane_object.scale = (video_plane_scale[0], -video_plane_scale[1], 1.0)
    video_capture.release()

    return video_plane_object


def create_constant_video_material(video_file_pathname: str) -> any:
    video_capture = cv2.VideoCapture(video_file_pathname)
    video_number_frames = video_capture.get(cv2.CAP_PROP_FRAME_COUNT)

    materials = bpy.data.materials
    material = materials.new('constant_video')
    material.use_nodes = True
    clear_material(material)

    nodes = material.node_tree.nodes
    links = material.node_tree.links

    texture = nodes.new(type='ShaderNodeTexImage')
    output = nodes.new(type='ShaderNodeOutputMaterial')
    links.new(texture.outputs['Color'], output.inputs['Surface'])

    texture.image = bpy.data.images.load(video_file_pathname)
    texture.image_user.use_auto_refresh = True
    texture.image_user.frame_duration = int(video_number_frames)

    return material


def sample_coordinate_frame(joints_filename, joints_animation_pca=None):
    """

    @param joints_animation_pca:
    @param joints_filename:
    @return:
    """
    bpy.ops.object.empty_add(type='ARROWS', location=(0, 0, 0))
    bpy.context.active_object.name = joints_filename
    coordinate_frame = bpy.context.active_object
    if joints_animation_pca is not None:
        coordinate_frame.data['pca'] = 0

    return coordinate_frame


def vertical_component(joints_skeleton_material):
    """
    add visualization for vertical component of the root joint
    @param joints_skeleton_material:
    @return:
    """
    bpy.ops.mesh.primitive_cube_add(size=0.1, enter_editmode=False)
    vertical_component_cube = bpy.context.active_object
    vertical_component_cube.color = (1.0, 0.0, 0.0, 1.0)  # assign red color to cube
    vertical_component_cube.data.materials.append(joints_skeleton_material)
    vertical_component_cube.location[1] = 1.0

    for frame_index in range(number_animation_frames):
        vertical_component_cube.location[0] = vertical_component_animation[frame_index]
        vertical_component_cube.keyframe_insert(data_path="location", index=0, frame=frame_index)


def animated_joints_spheres(joints_skeleton_material):
    joints_spheres = [None] * H36M_NUMBER_JOINTS
    joints_colors = get_h36m_joints_colors()
    joints_names = get_h36m_joints_names()

    for joint_index in range(H36M_NUMBER_JOINTS):
        bpy.ops.mesh.primitive_ico_sphere_add(radius=0.025)
        bpy.ops.object.shade_smooth()
        bpy.context.active_object.color = joints_colors[joint_index]
        bpy.context.active_object.name = joints_names[joint_index]
        bpy.context.active_object.data.materials.append(joints_skeleton_material)
        joints_spheres[joint_index] = bpy.context.active_object

        for frame_index in range(number_animation_frames):
            joints_spheres[joint_index].location = animated_skeletons[frame_index, joint_index, :]
            joints_spheres[joint_index].keyframe_insert(data_path="location", frame=frame_index)
        joints_spheres[joint_index].parent = parent_coordinate_frame


def skeleton_video_sample(video_filename: str, joints_skeleton_material: any,
                          videos_folder: Optional[str] = None, joints_3d_animations_folder: str | None = None,
                          joints_3d_stacked_animations_folder: str | None = None, joints_3d_animation_pca_folder: str | None = None) -> Type[
    bpy.context.active_object]:
    video_filename_base = os.path.splitext(video_filename)[0]
    joints_filename = video_filename_base + '.npy'
    animated_skeletons_filepath = os.path.join(joints_3d_animations_folder, joints_filename)
    video_filepath = os.path.join(videos_folder, video_filename)
    animated_skeletons_pca_filepath = os.path.join(joints_3d_animation_pca_folder, joints_filename)

    if not os.path.exists(animated_skeletons_filepath):
        exit()

    parent_coordinate_frame = sample_coordinate_frame(joints_filename)
    joints_spheres = animated_joints_spheres(joints_skeleton_material)

    if joints_3d_stacked_animations_folder is not None:
        vertical_component_cube = vertical_coponent(joints_skeleton_material)

    animated_skeletons = np.load(animated_skeletons_filepath)
    if joints_3d_animation_pca_folder is not None:
        animated_skeletons_pca = np.load(animated_skeletons_pca_filepath)

    vertical_component_animation = animated_skeletons[:, -1]
    number_animation_frames = vertical_component_animation.shape[0]
    animated_skeletons = animated_skeletons[:, :-1].reshape(-1, 16, 3)
    root_joint_animation = np.zeros((number_animation_frames, 1, 3))
    animated_skeletons = np.hstack((root_joint_animation, animated_skeletons))
    number_animation_frames = animated_skeletons.shape[0]
    number_skeleton_joints = H36M_NUMBER_JOINTS  # 17 for H3.6M
    initial_joints_location = animated_skeletons[0]

    print(BColors.OKBLUE + BColors.BOLD, 'filename:', BColors.ENDC,
          BColors.OKGREEN, joints_filename, BColors.ENDC,
          BColors.OKBLUE + BColors.BOLD, '; number skeleton frames:', BColors.ENDC,
          BColors.WARNING, animated_skeletons.shape[0], BColors.ENDC)

    bpy.ops.object.empty_add(type='ARROWS', location=(0, 0, 0))
    bpy.context.active_object.name = joints_filename
    parent_coordinate_frame = bpy.context.active_object
    parent_coordinate_frame.data['pca_1d'] = 0

    joints_spheres = [None] * number_skeleton_joints
    joints_colors = get_h36m_joints_colors()
    joints_names = get_h36m_joints_names()

    bpy.context.scene.frame_start = 0
    bpy.context.scene.frame_end = number_animation_frames
    bpy.context.scene.frame_set(0)

    if add_video_plane:
        video_plane = get_video_plane(video_filepath)
        video_plane_material = create_constant_video_material(video_filepath)
        video_plane.data.materials.append(video_plane_material)
        video_plane.parent = parent_coordinate_frame

    skeleton_curves = get_h36m_skeleton_curves(initial_joints_location, joints_colors)
    parent_skeleton_curves(skeleton_curves, parent_coordinate_frame)
    assign_material_skeleton_curves(skeleton_curves, joints_skeleton_material)

    for joint_index in range(number_skeleton_joints):
        bpy.ops.mesh.primitive_ico_sphere_add(radius=0.025)
        bpy.ops.object.shade_smooth()
        bpy.context.active_object.color = joints_colors[joint_index]
        bpy.context.active_object.name = joints_names[joint_index]
        bpy.context.active_object.data.materials.append(joints_skeleton_material)
        joints_spheres[joint_index] = bpy.context.active_object

        for frame_index in range(number_animation_frames):
            joints_spheres[joint_index].location = animated_skeletons[frame_index, joint_index, :]
            joints_spheres[joint_index].keyframe_insert(data_path="location", frame=frame_index)
        joints_spheres[joint_index].parent = parent_coordinate_frame

    for frame_index in range(number_animation_frames):
        if use_pca_skeletons:
            parent_coordinate_frame.data['pca_1d'] = animated_skeletons_pca[joint_index]
            parent_coordinate_frame.keyframe_insert(data_path="pca_1d", frame=frame_index)
        vertical_component_cube.location[0] = vertical_component_animation[frame_index]
        vertical_component_cube.keyframe_insert(data_path="location", index=0, frame=frame_index)
    vertical_component_cube.parent = parent_coordinate_frame

    bpy.context.scene.frame_set(0)
    hook_skeleton_h36m_curves(skeleton_curves, joints_spheres)
    parent_coordinate_frame.rotation_euler = (0, -0.5 * np.pi, 0)
    return parent_coordinate_frame


def main():
    root_folder = '/media/anton/4c95a564-35ea-40b5-b747-58d854a622d0/home/anton/work/fitMate/datasets/squats_2022_skeletons/results_base_video_mp3'

    video_folder = 'final_videos'
    joints_folder = 'joints3d'
    joints_stacked_folder = 'joints3d_stacked_procrustes'
    joints_pca_folder = 'joints3d_pca'

    use_video_filenames = False
    use_skeletons_animation_filenames = True
    use_skeletons_stacked_animation_filenames = True
    use_skeletons_pca_animation_filenames = True

    grid_width = 6
    item_space = 3.0

    videos_path = os.path.join(root_folder, video_folder)
    skeletons_path = os.path.join(root_folder, joints_folder)
    skeletons_stacked_path = os.path.join(root_folder, joints_stacked_folder)
    skeletons_pca_path = os.path.join(root_folder, joints_pca_folder)

    video_filenames = [f for f in os.listdir(joints_stacked_folder) if f.endswith('.npy')].sort()
    skeletons_filenames = [f for f in os.listdir(joints_folder) if f.endswith('.npy')].sort()
    skeletons_stacked_filenames = [f for f in os.listdir(joints_stacked_folder) if f.endswith('.npy')].sort()
    skeletons_pca_filenames = [f for f in os.listdir(joints_pca_folder) if f.endswith('.npy')].sort()

    if use_video_filenames and len(video_filenames):
        filenames = video_filenames
    elif use_skeletons_animation_filenames and len(skeletons_filenames):
        filenames = skeletons_filenames
    elif use_skeletons_stacked_animation_filenames and len(skeletons_stacked_filenames):
        filenames = skeletons_stacked_filenames
    elif use_skeletons_pca_animation_filenames and len(skeletons_pca_filenames):
        filenames = skeletons_pca_filenames
    else:
        filenames = []

    print(filenames)
    joints_skeleton_material = create_joints_skeleton_material()

    for index, filename in enumerate(filenames):
        grid_index_row = index // grid_width
        grid_index_column = index - grid_width * grid_index_row

        # sample_frame = skeleton_video_sample(videos_path, joints_stacked_folder, joints_pca_folder,
        #                                      filename, joints_skeleton_material, use_stacked_skeletons=False, use_pca_skeletons=True)
        filename_base = os.path.splitext(filename)[0]
        video_filename = filename_base + '.mp4'
        skeleton_animation_filename = filename_base + '.npy'
        skeleton_stacked_animation_filename = filename_base + '.npy'
        skeleton_animation_pca_filename = filename_base + '.npy'

        video_filepath = os.path.join(videos_path, video_filename)
        skeleton_animation_filepath = os.path.join(skeletons_path, skeleton_animation_filename)
        skeleton_animation_stacked_filepath = os.path.join(skeletons_stacked_path, skeleton_stacked_animation_filename)
        skeleton_animation_pca_filepath = os.path.join(skeletons_pca_path, skeleton_animation_pca_filename)

        sample = H36mSkeletonAnimationSample(video_filepath=video_filepath,
                                             skeleton_animation_filepath=skeleton_animation_filepath,
                                             skeleton_animation_stacked_filepath=skeleton_animation_stacked_filepath,
                                             skeleton_animation_pca_filepath=skeleton_animation_pca_filepath)

        sample_frame = sample.get()
        sample_frame.location[1] = item_space * grid_index_column
        sample_frame.location[2] = item_space * grid_index_row


if __name__ == '__main__':
    main()
