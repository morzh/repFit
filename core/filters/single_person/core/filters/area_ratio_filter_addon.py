...
class AreaRatioFilterAddon(MultiPersonsFilterAddonBase):
    ...
    def process(self, multi_persons_track: MultiplePersonsTracks):
        ...
        :param multi_persons_track: areas ratio
        """
        persons_number = len(multi_persons_track.persons)
        persons_areas = np.zeros(persons_number)
        for id_person in range(persons_number):
            persons_areas[id_person] = multi_persons_track.persons[id_person].mean_area()
        ...
        for key in small_persons_indices:
            del multi_persons_track.persons[key]