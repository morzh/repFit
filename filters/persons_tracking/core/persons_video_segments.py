...
from filters.persons_tracking.core.person_video_segments import PersonVideoSegments


class PersonsVideoSegments:
    def __init__(self, fps=30):
        self.fps = fps
        self.persons: dict[int, PersonVideoSegments] = {}
    ...
    def update(self, data: torch.Tensor, frame_number: int):
        for index in range(data.shape[0]):
            ...
            if current_person_id not in self.persons:
                ...
                self.persons[current_person_id] = PersonVideoSegments(current_person_id)
