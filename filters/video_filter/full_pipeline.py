...
def first_step():
    ...
    run_pool_steady_camera_filter(extract_stable_tracks, base_videos)
    run_pool_steady_camera_filter(cut_video_by_yolo_boxes, base_videos)

    cut_videos = list(CUT_VIDEO_DPATH.glob('*'))
    run_pool_steady_camera_filter(make_mp_joints, cut_videos)
